...
export default function PublicCollectionPage({ params }: { params: { id: string } }) {
    ...
    const fetchInstrument = async () => {
      try {
        ...
        if (docSnap.exists()) {
          ...
          // 🚨 SMART PARSER: Extracts instructions and individual questions
          parseQuestionnaireText(data.previewText, data.questionCount);
        } else {
  ...
  const parseQuestionnaireText = (text: string, count: number) => {
    // Look for the first question (e.g., "1. ")
    const firstQMatch = text.match(/\b1\.\s/);
    
    if (!firstQMatch) {
      // Fallback if AI formatting was weird: put everything in preamble, generate generic questions
      setPreamble(text);
